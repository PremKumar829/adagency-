...
export const ServicesGrid: React.FC<ServicesGridProps> = ({
  ...
  const tabs = [
    { id: 'all', label: t.services.tags.all },
    { id: 'telegram', label: t.services.tags.telegram },
    { id: 'gambling', label: t.services.tags.gambling },
    { id: 'social', label: t.services.tags.social },
    { id: 'crypto', label: t.services.tags.crypto },
    { id: 'custom', label: t.services.tags.custom },
  ];

  return (
    <section id="services" className="py-16 sm:py-24 bg-[#080C14] relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {tabs.map((tab) => {
            ...
            return (
              <motion.button
                ...
                className={`relative px-4 py-2 rounded-xl text-xs sm:text-sm font-semibold transition-colors cursor-pointer ${
                  isActive
        ...
        {/* Services Cards Grid with staggered animated transitions */}
        <motion.div
          layout
          ...
          <AnimatePresence mode="popLayout">
            {filteredServices.map((service, idx) => (
              <motion.div
                ...
                className="flex flex-col justify-between p-6 rounded-2xl bg-slate-900/90 border border-slate-800 hover:border-emerald-500/40 shadow-xl relative group smooth-card transform-gpu"
              >
                {/* Badge if present */}
                {service.badge && (
